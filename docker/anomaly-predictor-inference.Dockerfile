...
WORKDIR $HOME_DIR
RUN mkdir anomaly-predictor
WORKDIR $HOME_DIR/anomaly-predictor
RUN mkdir data models conf

COPY --chown=2222:2222 scripts/inference.sh scripts/
COPY --chown=2222:2222 src/anomaly_predictor src/anomaly_predictor
RUN touch src/__init__.py 

RUN chown 2222:2222 $HOME_DIR && \
    chown 2222:2222 $HOME_DIR/anomaly-predictor && \
    chown 2222:2222 $HOME_DIR/.bashrc && \
    chown 2222:2222 $HOME_DIR/anomaly-predictor/data && \
    chown 2222:2222 $HOME_DIR/anomaly-predictor/models && \
    chown 2222:2222 $HOME_DIR/anomaly-predictor/conf && \
